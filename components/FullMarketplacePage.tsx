...
const FullMarketplacePage: React.FC<FullMarketplacePageProps> = ({ onExit }) => {
      ...
      case 'favorites':
        return (
          <div className="animate-fade-in px-5 md:px-0 space-y-8">
            ...
            {favorites.length === 0 ? (
              <div className="py-24 text-center bg-white/40 backdrop-blur-md rounded-[3rem] border border-white/60 shadow-sm">
                ...
                <button onClick={() => setView('discover')} className="mt-6 px-8 py-3 bg-blue-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-700 transition-all">Start Exploring</button>
              </div>
            ) : (
      ...
      case 'add':
        return (
          <div className="animate-scale-up px-5 md:px-0 pb-32 max-w-2xl mx-auto space-y-8">
            ...
            <div className="bg-white/70 backdrop-blur-2xl p-8 md:p-12 rounded-[3rem] border border-white shadow-2xl shadow-slate-200/50 space-y-8">
               <div className="aspect-video bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center text-slate-400 hover:border-blue-400 hover:text-blue-500 transition-all cursor-pointer group">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <input type="text" placeholder="e.g. Siemens S7-1200 Controller" className="w-full p-5 bg-slate-50 border-none rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-blue-400/20" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Market Price (USD)</label>
                    <input type="number" placeholder="0.00" className="w-full p-5 bg-slate-50 border-none rounded-2xl font-bold text-sm outline-none" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Logistics Grade</label>
                    <select className="w-full p-5 bg-slate-50 border-none rounded-2xl font-bold text-sm outline-none appearance-none cursor-pointer">
                  ...
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <textarea placeholder="Describe technical specifications..." className="w-full p-5 bg-slate-50 border-none rounded-2xl font-bold text-sm h-32 resize-none outline-none"></textarea>
                  </div>
                  <button className="md:col-span-2 py-6 bg-blue-600 text-white rounded-[1.5rem] font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-500/20 hover:bg-blue-700 transition-all active:scale-95">
                    Sync to Network
                  </button>
               </div>
            </div>
          ...
          <div className="animate-fade-in px-5 md:px-0 max-w-4xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               <div className="lg:col-span-1 space-y-3 hidden lg:block">
                  {[1,2,3].map(i => (
                    <div key={i} className="p-4 bg-white border border-slate-100 rounded-2xl cursor-pointer hover:bg-blue-50 transition-all">
               ...
               <div className="lg:col-span-2 space-y-3">
                 {[1, 2, 3].map(i => (
                   <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-50 flex items-center gap-6 shadow-sm hover:shadow-md transition-all active:bg-slate-50 cursor-pointer">
                      <img src={`https://picsum.photos/id/${80+i}/100/100`} className="w-16 h-16 rounded-2xl object-cover grayscale opacity-70" />
          ...
          <div className="animate-fade-in px-5 md:px-0 max-w-5xl mx-auto space-y-12">
            <div className="flex flex-col md:flex-row items-center gap-10 md:text-left text-center bg-white/40 p-10 rounded-[3rem] border border-white">
               <div className="relative">
                 ...
                 <div className="absolute -bottom-2 -right-2 bg-blue-600 text-white p-3 rounded-2xl border-4 border-white shadow-xl"><Check size={24} strokeWidth={3} /></div>
               </div>
               <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap items-center md:justify-start justify-center gap-4 pt-2">
                    <span className="text-xs font-black text-blue-600 uppercase tracking-[0.2em] bg-blue-50 px-4 py-2 rounded-xl">Verified Enterprise</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {['Active Adverts', 'Network Purchases', 'Logistics Wallet', 'Compliance Docs', 'Account Settings', 'System Console'].map(item => (
                <button key={item} className="p-6 bg-white/60 backdrop-blur-md rounded-2xl border border-white text-left font-black text-xs uppercase tracking-widest text-slate-600 hover:bg-white hover:text-blue-600 transition-all flex items-center justify-between group shadow-sm">
                  {item} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                </button>
              ))}
              <button onClick={onExit} className="p-6 bg-slate-950 text-white rounded-2xl border-none text-left font-black text-xs uppercase tracking-widest hover:bg-blue-600 transition-all flex items-center justify-between group md:col-span-1 shadow-xl">
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-inter">
      {/* RESPONSIVE HEADER */}
      <header className="sticky top-0 z-[110] bg-white/60 backdrop-blur-2xl border-b border-white/20">
        <div className="max-w-[1600px] mx-auto px-6 py-4 md:py-6 flex items-center justify-between gap-4 md:gap-12">
          ...
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => changeView('discover')}>
            <div className="w-10 h-10 md:w-14 md:h-14 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-xl shadow-blue-500/20 group-active:scale-95 transition-all">N</div>
            <div className="hidden md:block">
              <h1 className="text-xl font-black text-slate-900 tracking-tighter leading-none">Nexus<span className="text-blue-500">Market</span></h1>
          ...
          <div className="flex-1 max-w-2xl relative group">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-600 transition-colors" size={20} strokeWidth={2} />
            <input 
              type="text" 
              placeholder="Search assets..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-slate-100/50 border-none rounded-[1.25rem] md:rounded-[2rem] py-3 md:py-5 pl-14 pr-14 text-sm font-bold text-slate-800 placeholder:text-slate-400 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none"
            />
          ...
          <div className="flex items-center gap-2 md:gap-6">
            ...
            <button className="relative p-2 text-slate-800 active:scale-90 transition-all">
              ...
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-blue-600 text-white text-[9px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-lg">2</span>
            </button>
            <button onClick={onExit} className="hidden md:flex items-center gap-2 px-6 py-3 bg-slate-950 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 transition-all shadow-xl shadow-slate-900/10 active:scale-95">
              WMS Console
            </button>
          </div>
        </div>
      ...
      {/* Main Content Area */}
      <main className="flex-1 w-full max-w-[1600px] mx-auto py-8 md:py-16 mb-24 md:mb-0">
        {renderContent()}
      </main>

      {/* MOBILE BOTTOM NAVIGATION */}
      <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[92%] h-18 bg-white/80 backdrop-blur-2xl border border-white/60 rounded-[2.5rem] shadow-2xl flex items-center justify-around px-4">
        {[
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 transition-all ${btn.primary ? '-top-4 relative' : ''} ${view === btn.id ? (btn.id === 'favorites' ? 'text-red-500' : 'text-blue-600') : 'text-slate-400'}`}
          >
            <div className={`${btn.primary ? 'w-14 h-14 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-2xl shadow-blue-500/30 active:scale-90 border-4 border-white transition-transform' : ''}`}>
              {btn.icon}
            </div>
