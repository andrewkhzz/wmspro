...
const DashboardStats: React.FC<DashboardStatsProps> = ({ onNavigateMarketplace }) => {
         ...
         <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-12 max-w-[1800px] mx-auto">
            <div className="space-y-6 max-w-2xl">
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-xs font-black uppercase tracking-[0.4em] text-blue-400">Logistics Node Active</span>
                </div>
                <h1 className="text-5xl md:text-7xl font-black text-white leading-[1.05] tracking-tighter">
                   Ultimate <span className="text-blue-500">Warehouse</span> Intelligence.
                </h1>
                <p className="text-xl text-slate-400 font-medium leading-relaxed">
                   Synchronizing {stats.totalItems} global assets across {stats.totalItems > 100 ? 'multimodal' : 'central'} distribution nodes. Performance is currently <span className="text-emerald-400 font-black">+14.2% optimized</span>.
                </p>
                <div className="flex flex-wrap gap-4 pt-4">
                    <button className="bg-blue-600 text-white px-10 py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-blue-500/30 hover:bg-blue-700 transition-all active:scale-95">
                        Initialize Smart Intake
                    </button>
                    <button onClick={() => onNavigateMarketplace?.()} className="bg-white/10 backdrop-blur-xl text-white border border-white/20 px-10 py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white/20 transition-all flex items-center gap-3">
                        Procure Surplus <ArrowUpRight size={16} />
                    </button>
                </div>
            </div>

            <div className="grid grid-cols-2 gap-4 w-full md:w-[480px]">
                <div className="bg-white/5 backdrop-blur-3xl border border-white/10 p-6 rounded-[2rem] hover:bg-white/10 transition-all">
                    <div className="flex justify-between items-start mb-4">
                        <DollarSign className="text-emerald-400" size={24} />
                        <TrendingUp className="text-emerald-500" size={14} />
                    </div>
                    <p className="text-3xl font-black text-white">${(stats.totalValue / 1000).toFixed(1)}k</p>
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mt-1">Grid Value</p>
                </div>
                <div className="bg-white/5 backdrop-blur-3xl border border-white/10 p-6 rounded-[2rem] hover:bg-white/10 transition-all">
                    <div className="flex justify-between items-start mb-4">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mt-1">Active SKUs</p>
                </div>
                <div className="bg-white/5 backdrop-blur-3xl border border-white/10 p-6 rounded-[2rem] hover:bg-white/10 transition-all">
                    <div className="flex justify-between items-start mb-4">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mt-1">Critical Nodes</p>
                </div>
                <div className="bg-white/5 backdrop-blur-3xl border border-white/10 p-6 rounded-[2rem] hover:bg-white/10 transition-all">
                    <div className="flex justify-between items-start mb-4">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mt-1">Sync Uptime</p>
                </div>
            </div>
         </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start max-w-[1800px] mx-auto w-full">
            <div className="lg:col-span-2 space-y-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <h3 className="text-2xl font-black text-slate-900 tracking-tight">Stock Velocity Analytics</h3>
                        <p className="text-sm font-medium text-slate-400 mt-1">Live visualization of node throughput vs storage capacity</p>
                    </div>
                    <div className="flex bg-slate-100 p-1.5 rounded-xl border border-slate-200">
                        <button className="px-4 py-1.5 bg-white shadow-sm rounded-lg text-[10px] font-black uppercase tracking-widest text-blue-600 transition-all">Weekly</button>
                        <button className="px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-all">Monthly</button>
                    </div>
                </div>

                <div className="h-[400px] w-full bg-slate-50/50 rounded-[2.5rem] p-8 border border-slate-100 relative">
          ...
          <div className="space-y-10 max-w-[1800px] mx-auto w-full pt-12 border-t border-slate-100">
             <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                   <div className="relative group">
                      ...
                      <input 
                        type="text" 
                        placeholder="Filter live assets..." 
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="bg-slate-50 border border-slate-100 rounded-xl py-2.5 pl-11 pr-4 text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all w-48 focus:w-72" 
                      />
                   </div>
             ...
             <div className={viewMode === 'grid' 
                ...
                {filteredLiveItems.map((item) => (
                   <div key={item.id} className={`group bg-white rounded-[2rem] border border-slate-100 transition-all hover:shadow-2xl hover:-translate-y-1 flex flex-col ${viewMode === 'list' ? 'md:flex-row p-4 gap-8' : ''}`}>
                      <div 
                        className={`relative overflow-hidden bg-slate-50 rounded-[1.75rem] cursor-pointer ring-inset ring-1 ring-slate-100 ${viewMode === 'list' ? 'w-48 h-48 shrink-0' : 'aspect-square'}`}
                      ...
                      >
                         <div className="absolute inset-0 bg-blue-600/0 group-hover:bg-blue-600/10 transition-colors z-10 flex items-center justify-center opacity-0 group-hover:opacity-100">
                            <div className="bg-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest text-blue-600 shadow-xl border border-white">View Detail</div>
                         </div>
                         ...
                         <div className="absolute top-4 right-4 p-2.5 bg-white/20 backdrop-blur-md rounded-xl border border-white/40 opacity-0 group-hover:opacity-100 transition-all z-20">
                      ...
                      <div className={`flex flex-col p-6 flex-1 ${viewMode === 'list' ? 'py-2 justify-between' : ''}`}>
                         ...
                         {viewMode === 'list' && (
                            <p className="text-xs text-slate-500 leading-relaxed font-medium line-clamp-2 max-w-2xl mt-4">Verified logistics SKU: {item.inventory_number}. Current status marked as {item.status.toUpperCase()}. Node RU-HUB-01 confirms full traceability.</p>
                         )}
                      ...
                      {viewMode === 'list' && (
                         <div className="flex items-center px-4">
                            <button onClick={() => onNavigateMarketplace?.(item.id)} className="px-8 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all">
                               Manage SKU
                            </button>
                         </div>
                      )}
